...
class PageComponent {
    ...
    final static COMP_TYPE_XE_DROPDOWN = "xeDropdown"
    ...
    final static COMP_ITEM_TYPES = [COMP_TYPE_LITERAL,COMP_TYPE_DISPLAY,COMP_TYPE_TEXT,COMP_TYPE_TEXTAREA,COMP_TYPE_NUMBER,
                                    COMP_TYPE_DATETIME,COMP_TYPE_EMAIL,COMP_TYPE_TEL,COMP_TYPE_LINK,COMP_TYPE_BOOLEAN, COMP_TYPE_XE_TEXT_BOX, COMP_TYPE_XE_DROPDOWN,
                                    COMP_TYPE_XE_BUTTON]
    ...
    final static COMP_DATASET_TYPES = [COMP_TYPE_GRID,COMP_TYPE_HTABLE,COMP_TYPE_LIST,COMP_TYPE_SELECT,COMP_TYPE_DETAIL,COMP_TYPE_DATA, COMP_TYPE_RADIO, COMP_TYPE_XE_DROPDOWN]

    // Data Set types only for display
    final static COMP_DATASET_DISPLAY_TYPES = [COMP_TYPE_SELECT,COMP_TYPE_RADIO, COMP_TYPE_XE_DROPDOWN]
    ...
    final static COMP_VISUAL_TYPES = [COMP_TYPE_PAGE,COMP_TYPE_FORM, COMP_TYPE_BLOCK, COMP_TYPE_LITERAL,
            ...
            COMP_TYPE_GRID,COMP_TYPE_HTABLE,COMP_TYPE_LIST,COMP_TYPE_SELECT,COMP_TYPE_DETAIL,COMP_TYPE_RADIO, COMP_TYPE_XE_TEXT_BOX, COMP_TYPE_XE_DROPDOWN,
            COMP_TYPE_XE_BUTTON]
    ...
    */
    ...
    def listCompile(int depth=0) {
        ...
        idTxtParam = "-{{\$index}}"
        if (label)
            txt += """<label class="${type} label $labelStyle" ${getIdAttr('label')} >${tran("label")}</label>"""
        // handle click event
        def click_txt=""
        if (onClick)
    ...
    String compileItem(String t, int depth=0){
        ...
        switch (typeInternal) {
            case COMP_TYPE_SELECT:
                ...
                result = """
                           |<select  ${getIdAttr(idTxtParam)} $autoStyleStr ng-model="$ngModel" $ngChange  ${defaultValue()}
            ...
            case COMP_TYPE_RADIO:
                ...
                result = """
                  |<span class="$valueStyle" ng-repeat="$SELECT_ITEM in $arrayName" $initTxt >
        ...
        // TODO is parseVariable still working after using DataSet as generic data object?
            ...
            case COMP_TYPE_SUBMIT:
                result = """<input ${getIdAttr(idTxtParam)} $autoStyleStr  type="submit" value="${tran("label")}"/> """
